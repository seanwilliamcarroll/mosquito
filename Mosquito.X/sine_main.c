...
#define DEFAULT_OFF_PERIOD                 (0)
#define DEFAULT_ON_PERIOD                 (10)
...
void flashInit() {
  ...
  SETTINGS.VOLUME      =               MAX_VOLUME; // Set at max loudness
  SETTINGS.ON_PERIOD   =        DEFAULT_ON_PERIOD;
  SETTINGS.OFF_PERIOD  =       DEFAULT_OFF_PERIOD;
...
void __ISR(_TIMER_2_VECTOR, ipl2)T2Int(void) {
  ...
  if (sound_on && SETTINGS.VOLUME > 0){
    ...
    value = value - 2047;
    value = (value / MAX_VOLUME)*SETTINGS.VOLUME;
    value = value + 2047;
...
static PT_THREAD(protothread_frequency(struct pt *pt)) {
  PT_BEGIN(pt);
  while (1) {
    if (SETTINGS.ON_PERIOD == 0){
      PT_YIELD_TIME_msec(100);
      sound_on = 0;
    }
    else if (SETTINGS.OFF_PERIOD == 0){
      PT_YIELD_TIME_msec(100);
      sound_on = 1;
    }
    else {
      // Both are at least 1
      sound_on = 1;
      PT_YIELD_TIME_msec(SETTINGS.ON_PERIOD * 1000);
      sound_on = 0;
      PT_YIELD_TIME_msec(SETTINGS.OFF_PERIOD * 1000);
    }
...
static PT_THREAD(protothread_cmd(struct pt *pt)) {
  ...
  while (1) {
    ...
    param =  -1;
    sscanf(PT_term_buffer, "%s %d", cmd, &param);
    ...
    if (cmd[0] == 'h') {
      ...
      PT_SPAWN(pt, &pt_DMA_output, PT_DMA_PutSerialBuffer(&pt_DMA_output));
      sprintf(PT_send_buffer, "-- f <int [1..1000>: Sets fundamental frequency\n");
      PT_SPAWN(pt, &pt_DMA_output, PT_DMA_PutSerialBuffer(&pt_DMA_output));
      sprintf(PT_send_buffer, "-- v <int [0..100]>: Sets master volume\n");
      PT_SPAWN(pt, &pt_DMA_output, PT_DMA_PutSerialBuffer(&pt_DMA_output));
      sprintf(PT_send_buffer, "-- a <int [0..]>: Set on period (seconds)\n");
      PT_SPAWN(pt, &pt_DMA_output, PT_DMA_PutSerialBuffer(&pt_DMA_output));
      sprintf(PT_send_buffer, "-- z <int [0..]>: Set off period (seconds)\n");
      PT_SPAWN(pt, &pt_DMA_output, PT_DMA_PutSerialBuffer(&pt_DMA_output));
    ...
    if (cmd[0] == 'p') {
      ...
      PT_SPAWN(pt, &pt_DMA_output, PT_DMA_PutSerialBuffer(&pt_DMA_output));
      sprintf(PT_send_buffer, "-- f: %d\n", SETTINGS.FUNDAMENTAL);
      PT_SPAWN(pt, &pt_DMA_output, PT_DMA_PutSerialBuffer(&pt_DMA_output));
      sprintf(PT_send_buffer, "-- v: %d\n", SETTINGS.VOLUME);
      PT_SPAWN(pt, &pt_DMA_output, PT_DMA_PutSerialBuffer(&pt_DMA_output));
      sprintf(PT_send_buffer, "-- a: %d\n", SETTINGS.ON_PERIOD);
      PT_SPAWN(pt, &pt_DMA_output, PT_DMA_PutSerialBuffer(&pt_DMA_output));
      sprintf(PT_send_buffer, "-- z: %d\n", SETTINGS.OFF_PERIOD);
      PT_SPAWN(pt, &pt_DMA_output, PT_DMA_PutSerialBuffer(&pt_DMA_output));
    }

    // Set fundamental frequency
    if (cmd[0] == 'f' && param > 0 && param <= 1000) {
      SETTINGS.FUNDAMENTAL = param;
      recalculateHarmonics();
    }
    
    // Set master volume
    if (cmd[0] == 'v' && param >= 0 && param <= 100) {
      SETTINGS.VOLUME = param;
    }
    
    // Set on period
    if (cmd[0] == 'a' && param >= 0) {
      SETTINGS.ON_PERIOD = param;
    }
    
    // Set off period
    if (cmd[0] == 'z' && param >= 0) {
      SETTINGS.OFF_PERIOD = param;
    }
